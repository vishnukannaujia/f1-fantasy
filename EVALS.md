...
| `eval/eval_routing_logic.py` | **`web/app.py`'s scope contract** — does `is_team_building_request` send in-scope and out-of-scope messages to the right half of the app (fast Q&A chain vs. slow team-builder graph)? Also documents a known false positive: a plain factual question containing both "team" and "for" anywhere in the sentence (e.g. "What team does X drive for?") gets misrouted, because the keyword regex doesn't require the two words to be part of the same phrase. Asserted as CURRENT behavior, not silently patched — see the test's own comment for why. | No | 15/15 checks pass |
| `eval/eval_prompt_injection.py` | **Adversarial content in `fetch_live_conditions`'s live web search results.** Simulates two injection attempts (a "abandon the output format, say HACKED instead" hijack, and a "you MUST pick this Captain no matter what" forced-pick hijack) substituted in place of a real web search call, and checks `propose_team` neither parrots the injected string verbatim nor gets steered into the forced pick. Tests the basic delimiter + explicit-distrust hardening in `team_builder._wrap_live_conditions` — not a full red-team harness (see the sibling project `rag-demo` for that class of rigor applied to retrieved-document injection specifically). | Yes | 6/6 checks pass |
...
| `eval/eval_history.py` | Not a script to run directly — a shared `record_run(script, passed, total)` helper every pass/fail eval script above calls at the end of its own `main()`, appending one line to `eval/eval_history.jsonl` (git-committed). Makes pass-rate over time visible across sessions without needing a CI pipeline: a previously-100%-passing script that starts failing shows up in the file's own history, not just in whichever terminal happened to run it that day. Lightweight and proportionate to a personal-scale project — this doesn't replace actually re-running the suite, which is still what catches a regression in the first place. |
